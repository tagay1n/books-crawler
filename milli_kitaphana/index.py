...
def index():
    print("Creating new index of books")
    ...
    with Progress(
        ...
        while next_page:
            with requests.get(
                ...
                if not total_docs:
                    _div = soup.select_one('.search-nav__item.search-nav__item_sm-ta-c > .search-nav__text')
                    total_docs = int(_div.text.split(':')[1].strip())
                
                for book_card in soup.select('.list__col-text'):
                    title_elem = book_card.select_one('h3[class="list__title"] > a[class="list__title-link"]')
                    meta = {
                        "title": title_elem.text.strip().rstrip(" NEW!!!").rstrip(),
                    }
                ...
                current_page = int(pagination_info.select_one('.active').text.strip())
                if not total_pages:
                    total_pages = sorted([int(j) for j in [i.text.strip() for i in pagination_info.select("li")] if j.isdigit()], reverse=True)[0]
                ...
                progress.update(task, completed=current_page, total=total_pages)
