...
def download():
    ...
    not_downloaded_docs = {k: v for k,v in index.items() if v.get("downloaded", False) == False }
...
class ProgressWrapper():
    ...
    def _pop_if_many_tasks(self, queue_size = 16):
        ...
        if incomplete_tasks_count >= queue_size:
            ...
            to_remove = sorted(completed_tasks, key = lambda i: i.start_time, reverse=True)[queue_size - incomplete_tasks_count:]
...
def _get_details(context):
    ...
    work_dir = get_in_workdir(os.path.join("../__artifacts/milli.kitaphana", code))
...
def _get_dh_params(context):
    ...
    cipher = AES.new(aes_key, AES.MODE_CBC, iv=context['config']["aes"]["default_iv"])
    pem_private_key = unpad(cipher.decrypt(context['config']["priv_key"]), AES.block_size)

    # Loading server's private key from PEM format
    rsa_priv_key = serialization.load_pem_private_key(pem_private_key, password=None)
...
def _dh_key_exchange(context):
    ...
    dh_sig = _sign(context["rsa_priv_key"], raw_local_dh_pub_key, context['config']['sig_prefix'])
    ...
    with _request(method="POST", url=HOST + context['keyUrl'], data=form_data) as resp:
        ...
        cipher = AES.new(secret_as_bytes, AES.MODE_CBC, iv=context['config']["aes"]["default_iv"])
...
def _download_by_code(context):
    ...
    if os.path.exists(outline_path):
        with open(outline_path, "r", encoding="utf-8") as file:
            ...
            available_pages = sum([i['pagesCount'] for i in parts if i.get("url")])
    ...
    if texts_url := source_meta.get("texts", {}).get("url"):
        ...
        context["dec_texts_zip_path"] = _decrypt_file(context, texts_url, enc_texts_path)
    else:
    ...
    with ThreadPool(processes=8) as pool:
        ...
        enc_part_paths = pool.map(lambda en: _download_part_task(context, en[1], en[0], counter, parts_count), enumerate(parts))
        
    with ThreadPool(processes=4) as pool:
        # decrypt the parts
        counter = itertools.count()
        context['progress'].main(f"Decrypted (0/{parts_count}) parts")
        dec_part_paths = pool.map(lambda en: _decrypt_file_task(context, en[0], en[1], en[2], counter, parts_count), [i for i in enc_part_paths if i])
...
def _download_part(context, part):
    ...
    with _request(method="GET", url=url, stream=True) as response:
        ...
        with open(enc_zip_path, "wb") as enc_zip:
            ...
            context['progress']._aux.update(task, description=f"Downloaded {part}")
...
def _decrypt_file(context, part, enc_unzip_dir):
    ...
    with open(enc_file_path, "rb") as file:
        aes_key = AES.new(context["decryption_key"], AES.MODE_CBC, iv=context["decryption_key_iv"])
...
def _sign(priv_key, message, prefix):
    ...
    message_to_sign = hashlib.sha256(message).digest() + entropia + since + until
